...
class Window(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self.initUi()
        self.initSignals()  # вызываем инициализацию сигналов (в методе initSignals)
    ...
    def initSignals(self) -> None:
        ...
        self.pushButtonComboBox.clicked.connect(self.onPushButtonComboBoxClicked)  # сигнал для вывода текста из comboBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonLineEdit.clicked.connect(self.onPushButtonLineEditClicked)  # сигнал для вывода текста из lineEdit
        self.pushButtonTextEdit.clicked.connect(self.onPushButtonTextEditClicked)  # сигнал для вывода текста из textEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonPlainTextEdit.clicked.connect(self.onPushButtonPlainTextEditClicked)  # сигнал для вывода текста из plaineTextEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonSpinBox.clicked.connect(self.onPushButtonSpinBoxClicked)  # сигнал для вывода значения из spinBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonDoubleSpinBox.clicked.connect(self.onPushButtonDoubleSpinBoxClicked)  # сигнал для вывода значения из doubleSpinBox в plainTextEditLog при нажатии на кнопку
        self.pushButtonTimeEdit.clicked.connect(self.onPushButtonTimeEditClicked)  # сигнал для вывода времени из timeEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonDateTimeEdit.clicked.connect(self.onPushButtonDateTimeEditClicked)  # сигнал для вывода времени из dateTimeEdit в plainTextEditLog при нажатии на кнопку
        self.pushButtonClearLog.clicked.connect(self.onPushButtonClearLogClicked)  # подключаем слот для очистки plainTextEditLog при нажатии на кнопку

        # для сигналов ниже используются уже реализованные слоты (onPushButtonComboBoxClicked, onPushButtonSpinBoxClicked и onPushButtonDateTimeEditClicked)
        self.comboBox.textActivated.connect(self.onPushButtonComboBoxClicked)  # сигнал для вывода текста в plainTextEditLog при изменении выбранного элемента в comboBox
        self.spinBox.editingFinished.connect(self.onPushButtonSpinBoxClicked)  # сигнал для вывода значения в plainTextEditLog при изменении значения в spinBox
        self.dateTimeEdit.dateTimeChanged.connect(self.onPushButtonDateTimeEditClicked)  # сигнал для вывода даты/времени в plainTextEditLog при изменении датывремени в dateTimeEdit
    ...
    def onPushButtonClearLogClicked(self) -> None:  # слот (метод) для очистки plainTextEditLog
        self.plainTextEditLog.clear()

    def onPushButtonComboBoxClicked(self) -> None:  # слот (метод) для вывода текста из comboBox
        self.plainTextEditLog.setPlainText(self.comboBox.currentText())

    def onPushButtonTextEditClicked(self) -> None:  # слот для вывода текста из textEdit
        self.plainTextEditLog.setPlainText(self.textEdit.toPlainText())

    def onPushButtonPlainTextEditClicked(self) -> None:  # слот для вывода текста из plainTextEdit
        self.plainTextEditLog.setPlainText(self.plainTextEdit.toPlainText())

    def onPushButtonSpinBoxClicked(self) -> None:  # слот для вывода текста из spinBox
        self.plainTextEditLog.setPlainText(self.spinBox.text())

    def onPushButtonDoubleSpinBoxClicked(self) -> None:  # слот для вывода текста из doubleSpinBox
        self.plainTextEditLog.setPlainText(self.doubleSpinBox.text())

    def onPushButtonTimeEditClicked(self) -> None:  # слот для вывода текста из timeEdit
        self.plainTextEditLog.setPlainText(self.timeEdit.text())

    def onPushButtonDateTimeEditClicked(self) -> None:  # слот для вывода текста из dateTimeEdit
